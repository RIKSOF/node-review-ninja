 ...
 *
 * @file OClint Checker for Objective-C programming language.
 */
 ...
 * Constructor
 *
 * @class [Checker OClintChecker]
 */
var checker = function OClintChecker() {
...
checker.prototype = {
   ...
   * @return {undefined}
   */
  reset: function OClintCheckerReset() {
    this.checkedFiles = [];
  },
   ...
   */
  doesValidate: function OClintCheckerDoesValidate( file ) {
    var validates = false;
    var included = ['.m', '.h'];
  ...
  * @return {undefined}
  */
  processFile: function OClintCheckerProcessFile( path, source, callback ) {
    var me = this;
    ...
    tmp.file({ postfix: '.m' }, function _tempFileCreated(err, tempPath, fd, cleanupCallback) {
      if (err) {
        logger.error(err);
      } else {
   ...
   * @return {undefined}
   */
  start: function OClintCheckerStart( from, baseSource, to, headSource, callback ) {
    ...
    var allDone = _.after( filesToCompare, function OClintCheckerFileDone() {
    ...
    this.processFile( from, baseSource, function OClintCheckerBaseSourceErrors( errs ) {
      errors.base = errs;
      allDone();
    });
    
    // Review the head source code.
    this.processFile( to, headSource, function OClintCheckerHeadSourceErrors( errs ) {
      errors.head = errs;
      allDone();
    });
  },
   ...
   * @return {undefined}
   */
  step: function OClintCheckerStep( change, path, position, callback ) {
    var comment = '';
   ...
   * @return {undefined}
   */
  done: function OClintCheckerDone( callback ) {
    var comment = '';
